...
public:
	...
	inline Option&			operator[]	(const char*	name);
	inline const Option&	operator[]	(const char*	name)	const;
...
public:
	enum ParsingResult {
		...
		CLPR_COMMAND_UNKNOWN_COMMAND,
		CLPR_COMMAND_UNKNOWN_OPTION,
		CLPR_COMMAND_OBLIGATORY_OPTION_NOT_SET,

		CLPR_OPTION_INVALID_VALUE,

		CLPR_GLOB_OBLIGATORY_OPTION_NOT_SET,

		CLPR_GLOB_EMPTY_LINE_AND_DEFAULT_COMMAND_INVALID_OR_NOT_SET
	};
...
public:
	...
	*/
	...
	Command&		operator[]	(char*	name);
	const Command&	operator[]	(char*	name)	const;
