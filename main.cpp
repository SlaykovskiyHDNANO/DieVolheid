...
/*void test2()
{
    ...
    GlobalAssistant* gas = new GlobalAssistant();
    Layer* l1 = new Layer("12345" , QVector<TimeSpan*>(), QVector<Layer*>(0), 0, "root", "im a root", gas);
    Layer* l2 = new Layer("123456" , QVector<TimeSpan*>(), QVector<Layer*>(0), 0, "son", "im a son", gas);
    l1->AddSublayer(l2);
    ...
    l2->AddInterval(x);
    l2->AddInterval(y);
    gas->AddTreeInTheForest(l1);
    QJsonObject ob;
    l1->Serialize(ob, 2, Layer::HEAVY_SERIALIZATION);
    Layer* l = new Layer();
    l->SetGlobalAssistant(gas);
